...
resnet = ResNetRegression(weighted=False).to(device)
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        if loss_val_et > avg_loss_validation:
          ...
          torch.save(resnet.state_dict(), "model_nw.pt")
          count = 0
        else:
          count += 1
...
loaded_resnet = ResNetRegression(weighted=False).to(device)
loaded_resnet.load_state_dict(torch.load("model_nw.pt"))
loaded_resnet.eval()
...
with torch.no_grad():
  for test_inputs, test_labels in test_loader:
      ...
      test_outputs = loaded_resnet(test_inputs)
